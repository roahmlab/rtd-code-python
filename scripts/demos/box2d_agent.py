if __name__ == '__main__':
    # import modules
    from rtd.demos.box2d import BoxAgent, BoxAgentInfo, BoxAgentVisual, BoxAgentClientVisual
    from rtd.entity.components import GenericEntityState
    from rtd.sim.systems.visual import PyvistaVisualSystem, ClientVisualSystem
    ...
    visual1 = BoxAgentClientVisual(info1, state1, edge_color=[1, 0, 0], edge_width=3, face_opacity=0.5)
    agent1 = BoxAgent(info=info1, state=state1, visual=visual1)
    
    info2 = BoxAgentInfo(width=2, height=0.5, color=[0, 1, 0])
    agent2 = BoxAgent(info=info2, visual=BoxAgentClientVisual)
    ...
    agent1.state.commit_state_data(1, [-8,-1])   # (-4,-1) at t=4
